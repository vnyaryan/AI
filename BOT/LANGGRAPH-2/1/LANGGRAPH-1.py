...
class OverallState(BaseModel):
    ...
    summary: str = ""
    node2_llm_decision: Literal["yes", "no"] = "yes"
...
# llm = ChatOpenAI(openai_api_key=OPENAI_API_KEY, temperature=0).bind_tools(tools)
...
def setup_logging():
    ...
    """
    try:
        log_directory = r'C:\Users\ARYAVIN\Documents\GitHub\AI\ai\BOT\LANGGRAPH-2\1'
...
def summarize_conversation(state: OverallState, llm):
    """
    Summarizes the entire conversation using the given `llm`.

    Args:
        state (OverallState): The current conversation state containing messages.
        llm: The ChatOpenAI instance used for generating the summary.
    """
    # Prepare the summarization prompt
    if state.summary:
        prompt_text = (
            f"This is the current summary of the conversation: {state.summary}\n\n"
            "Extend this summary based on the following new messages:\n"
            f"{[message.content for message in state.messages]}"
        )
    else:
        prompt_text = (
    ...
    # Create the SystemMessage for summarization
    summary_prompt = SystemMessage(content=prompt_text)
...
def node1(state: OverallState):
    ...
    try:
        ...
        summarize_conversation(state, llm)  # Call the summarize_conversation function
        logging.debug(f"Updated conversation summary: {state.summary}")
...
def node2(state: OverallState):
    ...
    """
    try:
        logging.debug("NODE2: EVALUATE_QUERY STARTED.")


        evaluation_prompt = ChatPromptTemplate.from_messages([
            SystemMessagePromptTemplate.from_template(
                "You are an intelligent assistant. Analyze the following conversation history and the latest human message. "
                "Determine if the latest human message is related to Azure az CLI commands. "
                "Respond with 'Yes' if it's related to Azure az CLI, or 'No' if not related to Azure az CLI."
            ),
        ...
        state.node2_llm_decision = llm_decision
        ...
        if llm_decision == "yes":
            ...
            sys.exit(f"Error in NODE2: {e}")


        logging.debug("NODE2: EVALUATE_QUERY COMPLETED.")
        return state
    except Exception as e:
        logging.error(f"Error in NODE2: EVALUATE_QUERY: {e}", exc_info=True)
        raise
...
def node3(state: OverallState):
    """
    Node 3:  refines the user query and prepares it for embedding generation.
    ...
    """
    try:
        logging.debug("NODE3: REFINE_QUERY STARTED.")
        ...
        if state.node2_llm_decision == "yes":
            logging.debug("LLM decided similarity search is required. Refining user query.")
            ...
            refinement_prompt = ChatPromptTemplate.from_messages([
                SystemMessagePromptTemplate.from_template(
                 "You are an intelligent assistant. Based on the conversation history, "
                    "refine the last human message to make it more specific and suitable for a similarity search."
        ...
        elif state.node2_llm_decision == "no":
            logging.debug("LLM decided similarity search is not required.")
            
            state.next_node = "ai_response"
        else:
            raise ValueError(f"Unexpected LLM response: {state.node2_llm_decision}")

        logging.debug("NODE3: REFINE_QUERY COMPLETED.")
        return state
    except Exception as e:
        logging.error(f"Error in NODE3: REFINE_QUERY: {e}")
        raise
...
def node4(state: OverallState):
    """
    Node 4: Generates and stores an embedding for the refined query from Node3.
    ...
    try:
        ...
        if not refined_query:
            raise ValueError("Refined query from Node3 is empty.")
        ...
        logging.error(f"Error in NODE4: {e}")
        raise
...
def node5(state: OverallState):
    ...
    try:
        ...
        logging.error(f"Error in SIMILARITY SEARCH NODE5: {e}")
        raise


def node6(state: OverallState):
    """
    Node 6: Generates a response for the user based on `retrieved_documents` 
    ...
    """
    try:
        logging.debug("NODE6 EXECUTION STARTED.")
        ...
        logging.debug("NODE6 EXECUTION COMPLETED.")
        return state
    except Exception as e:
...
def node7(state: OverallState):
    ...
    try:
        ...
        logging.debug("NODE7: RELEVANCE ASSESSMENT COMPLETED.")
        return state

    except Exception as e:
        logging.error(f"Error in NODE7: RELEVANCE ASSESSMENT: {e}")
        raise

def node8(state: OverallState):
    """
    Node8: Generates a clarification query when the AI response is assessed as 'BAD'.
    """
    try:
        logging.debug("NODE8: CLARIFICATION NODE STARTED.")
        ...
        logging.debug(f"AI: {clarification_question}")

        print(clarification_question)
        logging.debug("NODE8: CLARIFICATION NODE COMPLETED.")
        return state

    except Exception as e:
        logging.error(f"Error in NODE8: CLARIFICATION NODE: {e}")
        raise
...
def define_graph():
    ...
    builder.add_node("llm_decision", node2)                # Decide if similarity search is required using LLM
    builder.add_node("evaluate_similarity", node3)         # Refines user query if needed and prepares for embedding
    builder.add_node("generate_embedding", node4)          # Generates embedding if needed
    builder.add_node("similarity_search", node5)           # Performs similarity search
    builder.add_node("ai_response", node6)                 # Generates AI response
    builder.add_node("relevance_assessment", node7)        # Assess AI response relevance
    builder.add_node("clarification_node", node8)          # Ask for user clarification if AI response is not relevant


    # Add Edges
    builder.add_edge(START, "user_query")
    builder.add_edge("user_query", "llm_decision")
    builder.add_edge("llm_decision", "evaluate_similarity")
...
def main():
    ...
    try:
        ...
        while True:  # Infinite loop until explicit termination
            ...
            # Summarize the conversation at the end of each iteration
            summarize_conversation(initial_state, llm)
            logging.debug(f"Updated conversation summary: {initial_state.summary}")
