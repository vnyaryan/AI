...
2. Prompts the user to enter their Azure CLI task.
3. Generates an initial Azure CLI command based on the user's input.
4. Prompts the user for feedback to determine if the generated command is correct.
5. If the user provides feedback indicating the command needs changes, the script refines the command based on user clarification.
6. Repeats the feedback loop until the user approves the command.
7. Logs the success or failure of each operation and saves the conversation history.

Input Parameters:
1. API_KEY: The OpenAI API key for authentication (set as a constant at the start of the script).
2. USER_TASK: A string describing the Azure CLI task (input provided by the user).
...
# Generate an initial Azure CLI command based on the user input
def generate_azure_cli_command(user_task, llm, memory):
    logging.debug("Generating Azure CLI command for user task: %s", user_task)
    chat_prompt_template = ChatPromptTemplate.from_messages(
        [
            ...
            HumanMessagePromptTemplate.from_template("{user_task}")
        ]
    )
    azure_cli_request_prompt = chat_prompt_template.format_prompt(user_task=user_task).to_messages()
...
def main():
    ...
    azure_cli_command = generate_azure_cli_command(user_task, llm, memory)
