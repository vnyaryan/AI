...
def main():
    ...
    while user_feedback != "yes":
        print("\nDo you want to:")
        print("1. Change the previously generated command")
        print("2. Ask a question about the command shared previously")
        print("3. Exit")
        user_choice = input("Enter '1' for changing the command, '2' for asking a question, or '3' to exit: ").strip()

        if user_choice == '1':
            # User wants to change the command
            ...
            # User wants to ask about a parameter or detail of the command
            clarification = input("What would you like to know about the command?: ").strip()
            # Use TEST_INDEX for parameter-related queries to provide detailed information
            context = perform_similarity_search(clarification, embeddings_model, index_test)
            detailed_info = generate_azure_cli_command_with_context(f"Explain the parameter {clarification}", llm, context)
            print("\nParameter Details:")
            print(detailed_info)
        elif user_choice == '3':
            # Exit the loop
            print("Exiting the refinement process. Please review the command provided.")
            break
        else:
            print("Invalid choice. Please enter '1', '2', or '3'.")

        # Ask for feedback again to see if the user is satisfied, only if they haven't chosen to exit
        if user_choice != '3':
            user_feedback = get_user_feedback()
