...
def ask_human_review(tool_calls):
    """
    Asks the user to review and approve tool calls before execution.
    """
    if not tool_calls:
        #print("No tool calls detected. Skipping tool execution.")
...
builder.add_conditional_edges(
    "node2",
    lambda state: ask_human_review(state["messages"][-1].tool_calls)
)
