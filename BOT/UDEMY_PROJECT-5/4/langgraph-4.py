"""
Script to demonstrate the use of LangGraph with conditional edges.

INPUT:
- The script takes an input dictionary with the following structure:
  {
      "user_input": str  # A user-provided string input ("My" or "YOUR")
  }
...
3. **Routing Function**:
   - Evaluates the `user_input` field from Node 2's output:
     - Routes to **Node 3** if `user_input == "My"`.
     - Routes to **Node 4** if `user_input == "YOUR"`.
...
5. **Node 4**:
   - Processes the `PrivateState` from Node 2.
   - Appends " Warrior" to the `bar` value to produce the final `graph_output` field in the `OutputState`.
...
CONDITIONAL EDGES:
- The script demonstrates conditional edge routing using a routing function to dynamically choose the next node based on runtime state.

USAGE:
- This script demonstrates sequential and conditional state updates using LangGraph, showcasing logging, validation, conditional edges, and custom reducer functionality.
"""
...
def node_4(state: PrivateState) -> OutputState:
    try:
        ...
        validate_private_state(state)  # Validate input
        result = {"graph_output": state["bar"] + " Warrior"}  # Different output for node_4
        validate_output_state(result)  # Validate output
...
def routing_function(state: OverallState) -> str:
    logging.debug(f"Routing based on user_input: {state['user_input']}")
    if state["user_input"] == "My":
        return "node_3"
    elif state["user_input"] == "YOUR":
        return "node_4"
    else:
        raise ValueError(f"Invalid user_input: {state['user_input']}")

# Add Conditional Edges Using Proper Syntax
builder.add_conditional_edges(
    source="node_2",  # Source node
...
try:
    ...
    result = graph.invoke({"user_input": "YOUR"})  # Change to "YOUR" to test node_4
