...
def setup_logging():
    ...
    """
    try:
        log_directory = r'C:\Users\ARYAVIN\Documents\GitHub\AI\ai\BOT\LANGGRAPH-1\3'
...
def node2(state: OverallState):
    """
    Node 3: Generates and stores an embedding for the latest HumanMessage in the OverallState.
...
#     """
#     Generates a response for the user based on the retrieved documents and the latest user query.

#     Args:
#         state (OverallState): The current workflow state.

#     Returns:
#         OverallState: Updated state with the AI's response appended to messages.

#     Raises:
#         ValueError: If `retrieved_documents` is empty or no valid documents exist.
#     """
#     try:
...
#             raise ValueError("No retrieved documents found in OverallState. Ensure similarity_search_node runs first.")
...
#         # Format retrieved documents for inclusion in the prompt using `values`
#         document_context = "\n".join(
#             f"Document ID: {doc['id']}\nScore: {doc['score']}\nValues: {doc['values']}\nMetadata: {doc['metadata']}\n"
#             for doc in state.retrieved_documents
#         )
#         logging.debug(f"Formatted document context for prompt: {document_context}")


#         # Create the prompt for the LLM
#         prompt = f"""
#         You are an intelligent assistant. Based on the following retrieved documents, answer the user's query.
...
def node4(state: OverallState):
    ...
    Args:
        state (OverallState): The current workflow state.
        memory (ConversationSummaryMemory): Memory object to save the conversation context.
...
def define_graph():
    ...
    builder.add_node("generate_embedding", node2)
    builder.add_node("similarity_search", node3)
    builder.add_node("ai_response", node4)    
