...
def node3(state: MessagesState):
    """
    Node 3: Invokes the LLM only if a tool was actually invoked.
    """
    try:
        
        logging.debug("NODE3 EXECUTION STARTED.")
        
        # Check if any tool calls exist in the last message
        tool_calls_detected = any(
        ...
        # If tool calls exist, invoke the LLM
