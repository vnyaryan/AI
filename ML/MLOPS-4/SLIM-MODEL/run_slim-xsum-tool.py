...
# ----------------------------------------------------------------------------
# PERFORM MODEL INFERENCE (Summarization)
# ----------------------------------------------------------------------------
def perform_inference(model, text_sample):
    try:
        ...
        summary_output = response.get('llm_response', {})
        logging.info(f"Generated Summary: {summary_output}")
...
def main():
    ...
    try:
        ...
        summary_output = perform_inference(model, text_sample)

        # Display summary on the console
        print("\nExtracted Summary:")
        for sentence in summary_output.get("xsum", []):
            print(f"- {sentence}")
